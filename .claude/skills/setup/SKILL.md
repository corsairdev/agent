...
If `pnpm` isn't available, try `npm install`. If `@whiskeysockets/baileys` fails to build, install build tools first:
...
**If it fails:** Check that `DATABASE_URL` is in `.env` and Postgres is running (`pnpm db:up`).
...
```bash
pnpm whatsapp:auth --phone NUMBER &
AUTH_PID=$!
